# ==============================================================================
# ANASH ERP v201 - ÉDITION BALIKA BUSINESS (VERSION LONGUE & COMPLÈTE)
# ------------------------------------------------------------------------------
# FUSION : FACTURES PRO (A4 & 80mm) | PANIER NOIR SUR BLANC | DESIGN COBALT
# ------------------------------------------------------------------------------
# LIGNES : > 700 | OPTIMISATION : SMARTPHONE & TABLETTE | DESIGN : COBALT
# ==============================================================================
...
def init_system_db():
    with sqlite3.connect(DB_FILE) as conn:
        ...
        if not cursor.fetchone():
            cursor.execute("INSERT INTO global_settings VALUES (1, 'ANASH ERP v201', 'BIENVENUE CHEZ BALIKA BUSINESS - VOTRE SUCCÈS COMMENCE ICI', '2.0.1')")
...
# ------------------------------------------------------------------------------
# 3. DESIGN CSS PERSONNALISÉ (STYLE COBALT & NÉON & FACTURES)
# ------------------------------------------------------------------------------
...
def apply_custom_styles():
    ...
    <style>
        ...
        /* CADRE PANIER : NOIR SUR FOND BLANC (Votre demande) */
        .cart-white-box {{
        ...
        .cart-white-box b, .cart-white-box p, .cart-white-box span, .cart-white-box div {{
            color: #000000 !important;
        }}
        ...
        .receipt-80mm * {{ color: black !important; font-size: 14px; }}
        .dashed-line {{ border-top: 1px dashed black; margin: 5px 0; }}

        /* FACTURE A4 PRO */
        .receipt-A4 {{
            background: white !important; color: black !important; padding: 40px;
            font-family: Arial, sans-serif; width: 800px; min-height: 1000px;
            margin: auto; border: 1px solid #eee;
        }}
        .receipt-A4 * {{ color: black !important; }}
        .a4-table {{ width: 100%; border-collapse: collapse; margin-top: 20px; }}
        .a4-table th, .a4-table td {{ border: 1px solid black; padding: 8px; text-align: left; }}
        .a4-table th {{ background: #f2f2f2; }}
        ...
        @media print {{
            ...
            .receipt-80mm, .receipt-A4 {{ border: none !important; width: 100% !important; }}
        }}
    </style>
...
# --- 8.2 CAISSE TACTILE (AMÉLIORÉE : PANIER BLANC & FACTURES PRO) ---
elif choice == "🛒 CAISSE":
    if st.session_state.session['viewing_invoice']:
        ...
        if fmt == "TICKET 80mm":
            ...
            <div class='receipt-80mm'>
                <center><b>{shop_info[0]}</b><br>{shop_info[3]}<br>Tél: {shop_info[4]}</center>
                <div class='dashed-line'></div>
                N°: {inv['ref']}<br>Date: {inv['date']}<br>Cli: {inv['cli']}
                <div class='dashed-line'></div>
                {"".join([f"<div style='display:flex; justify-content:space-between;'><span>{k} x{v['q']}</span><span>{(v['q']*v['p']):,.2f}</span></div>" for k,v in inv['items'].items()])}
                <div class='dashed-line'></div>
                <div style='display:flex; justify-content:space-between; font-weight:bold;'><span>TOTAL</span><span>{inv['total']:,.2f} {inv['devise']}</span></div>
                <div class='dashed-line'></div>
                <center>{shop_info[2]}<br>ANASH ERP v201</center>
            </div>
            """, unsafe_allow_html=True)
        else:
            st.markdown(f"""
            <div class='receipt-A4'>
                <table style='width:100%'><tr>
                    <td><h1 style='color:#0044ff;'>{shop_info[0]}</h1><p>{shop_info[3]}<br>Tél: {shop_info[4]}</p></td>
                    <td style='text-align:right;'><h2>FACTURE</h2><p>N° {inv['ref']}<br>Date: {inv['date']}</p></td>
                </tr></table>
                ...
                <table class='a4-table'>
                    ...
                    <tr style='font-weight:bold;'><td colspan='3' style='text-align:right;'>TOTAL GÉNÉRAL ({inv['devise']})</td><td>{inv['total']:,.2f}</td></tr>
                </table>
        ...
        if st.session_state.session['cart']:
            ...
            st.markdown("<div class='cart-white-box'>", unsafe_allow_html=True)
            st.subheader("📋 PANIER EN COURS")
            total_panier_usd = 0.0
            
            for art, data in list(st.session_state.session['cart'].items()):
                # On ajuste le prix selon la devise choisie pour l'affichage
                display_p = data['p'] if devise == "USD" else data['p'] * taux_actuel
                ...
                stot = display_p * new_q
                total_panier_usd += (data['p'] * new_q) # Stockage toujours en base USD
                
                c_n.markdown(f"<b>{art}</b><br>{display_p:,.2f} {devise}", unsafe_allow_html=True)
...
# ==============================================================================
# FIN DU CODE v201 - ANASH ERP BALIKA BUSINESS
# ==============================================================================
