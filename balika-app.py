...
# ==========================================
# 1. CONFIGURATION SYSTÈME (v255 - FIX DB & MOBILE)
# ==========================================
st.set_page_config(page_title="BALIKA ERP v255", layout="wide", initial_sidebar_state="collapsed")
...
# ==========================================
# 2. INITIALISATION ET MISE À JOUR DB
# ==========================================
def init_db():
    ...
    # --- SÉCURITÉ : Mise à jour automatique des colonnes manquantes ---
    try:
        run_db("SELECT email FROM config LIMIT 1", fetch=True)
    except:
        run_db("ALTER TABLE config ADD COLUMN email TEXT DEFAULT 'info@mail.com'")
        run_db("ALTER TABLE config ADD COLUMN rccm TEXT DEFAULT '000'")
        run_db("ALTER TABLE config ADD COLUMN idnat TEXT DEFAULT '000'")
...
init_db()
# Chargement sécurisé
cfg_res = run_db("SELECT entreprise, message, taux, adresse, telephone, email, rccm, idnat FROM config WHERE id=1", fetch=True)
C_ENT, C_MSG, C_TAUX, C_ADR, C_TEL, C_MAIL, C_RCCM, C_IDN = cfg_res[0]

# ==========================================
# 3. DESIGN CSS MOBILE-FIRST (FORCÉ NOIR/BLANC)
# ==========================================
st.markdown(f"""
    <style>
    ...
    /* Force la visibilité sur iPhone */
    div[data-baseweb="select"] > div, .stSelectbox div, div[data-testid="stMarkdownContainer"] p {{
        background-color: #F8F9FA !important;
        color: #000000 !important;
        opacity: 1 !important;
    }}
    h1, h2, h3, label, p, span, div, b {{ color: #000000 !important; }}
    .stButton>button {{
        background: linear-gradient(to right, #FF8C00, #FF4500) !important;
        color: white !important; border-radius: 10px; height: 50px; font-weight: bold; border: none;
    }}
    .total-box {{
...
if not st.session_state.auth:
    st.markdown('<div style="background:#FF8C00; padding:30px; border-radius:15px; text-align:center; color:white !important;"><h1>ERP LOGIN</h1></div>', unsafe_allow_html=True)
...
with st.sidebar:
    ...
    if st.session_state.role == "ADMIN":
        pages.update({"📦 STOCK": "STOCK", "📊 RAPPORT": "RAPPORT", "👥 USERS": "USERS", "⚙️ CONFIG": "CONFIG"})
...
if st.session_state.page == "ACCUEIL":
    ...
    if not st.session_state.last_fac:
        ...
        if st.session_state.panier:
            ...
            for art, qte in list(st.session_state.panier.items()):
                ...
                new_q = c1.number_input(f"{art} ({pf:,.0f} {devise})", 1, p_map[art]['s'], value=qte, key=f"q_{art}")
                st.session_state.panier[art] = new_q
                if c2.button("🗑️", key=f"d_{art}"): del st.session_state.panier[art]; st.rerun()
            
            st.markdown(f'<div class="total-box">À PAYER : {total:,.2f} {devise}</div>', unsafe_allow_html=True)
        ...
        if f['fmt'] == "80mm":
            st.markdown(f'<div class="fac-80"><h3>{C_ENT}</h3><p>{C_ADR}<br>{C_TEL}</p><hr><p align="left">N°: {f["ref"]}<br>Client: {f["cl"]}<br>{f["date"]}</p><hr><table style="width:100%">{h_lines}</table><hr><h4>TOTAL: {f["tot"]:,.0f} {f["dev"]}</h4><p>Payé: {f["ac"]} | Reste: {f["re"]}</p></div>', unsafe_allow_html=True)
        else:
            st.markdown(f'<div class="fac-a4"><div style="display:flex; justify-content:space-between"><div><h1>{C_ENT}</h1><p>{C_ADR}<br>Tél: {C_TEL}<br>Email: {C_MAIL}<br>RCCM: {C_RCCM}</p></div><div style="text-align:right"><h2>FACTURE</h2><p>N° {f["ref"]}<br>{f["date"]}</p></div></div><hr><b>CLIENT : {f["cl"]}</b><table style="width:100%; border-collapse:collapse; margin-top:15px"><tr style="background:#EEE"><th style="border:1px solid #000">Art</th><th style="border:1px solid #000">Qté</th><th style="border:1px solid #000">Total</th></tr>{h_lines.replace("<td>", "<td style=\'border:1px solid #000; padding:8px\'>")}</table><h3 style="text-align:right">NET À PAYER : {f["tot"]:,.2f} {f["dev"]}</h3></div>', unsafe_allow_html=True)
        st.button("🖨️ IMPRIMER", on_click=lambda: st.markdown("<script>window.print();</script>", unsafe_allow_html=True))

elif st.session_state.page == "STOCK":
    st.title("📦 Inventaire")
    with st.form("p"):
        n, p = st.columns(2); nm = n.text_input("Désignation"); pr = p.number_input("Prix")
        d = st.selectbox("Devise", ["USD", "CDF"]); q = st.number_input("Qté Stock", 1)
        if st.form_submit_button("AJOUTER"):
            run_db("INSERT INTO produits (designation, stock_initial, stock_actuel, prix_vente, devise_origine) VALUES (?,?,?,?,?)", (nm.upper(), q, q, pr, d)); st.rerun()
    for r in run_db("SELECT * FROM produits", fetch=True):
        with st.expander(f"{r[1]} ({r[3]} en stock)"):
            np = st.number_input("Modifier Prix", value=float(r[4]), key=f"p_{r[0]}")
    ...
    for d in run_db("SELECT * FROM dettes", fetch=True):
        ...
        v = st.number_input("Payer tranche", 0.0, float(d[2]), key=f"v_{d[0]}")
        if st.button("Valider", key=f"b_{d[0]}"):
    ...
    with st.form("cfg"):
        ...
        tx = st.number_input("Taux (1 USD en CDF)", value=C_TAUX); ms = st.text_area("Marquee", C_MSG)
    ...
    if os.path.exists("anash_data.db"):
        with open("anash_data.db", "rb") as f: st.download_button("📥 Backup DB", f, file_name="backup.db")
